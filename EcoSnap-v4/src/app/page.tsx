...
const WASTE_POINTS: Record<AIWasteCategory, number> = {
  ewaste: 100,
  biowaste: 60,
  cardboard: 80,
  paper: 70,
...
const wasteCategoryFiveRTips: Record<AIWasteCategory | 'general', TipInfo> = {
  general: { // Corresponds to "AI-determined other" or a generic fallback
  ...
  plasticOther: { // This tip set is for the specific AI category 'plasticOther'
  ...
  other: { // Corresponds to "Trash" or AI-determined "other"
...
export default function HomePage() {
  ...
  const [currentUploadCategory, setCurrentUploadCategory] = useState<AIWasteCategory | 'general' | undefined>(undefined);
  ...
  useEffect(() => {
    console.log(">>> [EFFECT] checkLoginStatus running");
    const checkLoginStatus = () => {
      const loggedIn = localStorage.getItem('isLoggedIn') === 'true';
      setIsLoggedIn(loggedIn);
      console.log(">>> [EFFECT] isLoggedIn from localStorage:", loggedIn);
      ...
      if (loggedIn) {
        ...
        if (storedUserData.id === 'localUser' || (userEmail && storedUserData.email !== userEmail) || (userName && storedUserData.displayName !== userName) ) {
           ...
           console.log(">>> [EFFECT] User logged in, but local data needs sync or is default. Re-initializing profile for:", userEmail || "unknown email");
           storedUserData = {
            ...defaultUserProfile,
    ...
    window.addEventListener('storage', (event) => {
        if (event.key === USER_DATA_KEY || event.key === 'isLoggedIn' || event.key === HISTORY_STORAGE_KEY) {
            console.log(">>> [EFFECT] Storage event detected for key:", event.key);
            checkLoginStatus();
        }
    });
  ...
  const handleClassify = async (imageDataUri: string, categoryUserInitiatedWith?: AIWasteCategory | 'general'): Promise<ClassifyWasteOutput | null> => {
  ...
  const openUploadModalForCategory = (categoryId: AIWasteCategory | 'general' | undefined, categoryName: string) => {
    setClassificationError(null);
              ...
              <Dialog key={`top-${category.id}`} open={isUploadModalOpen && currentUploadCategory === category.id && currentUploadCategoryFriendlyName === category.name} onOpenChange={ open => {
                ...
                <DialogTrigger asChild>
                  <Card
                    ...
                    className="p-2 sm:p-2 md:p-3 flex flex-col items-center gap-1.5 sm:gap-1.5 md:gap-2.5 cursor-pointer hover:bg-muted/50 transition-colors shadow-sm w-[85px] sm:w-[100px] md:w-[140px] flex-shrink-0"
                  >
                     <ImageWithFallback
                        ...
                        sizes="(max-width: 639px) 40px, (max-width: 767px) 48px, 80px"
                        wrapperClassName="relative w-10 h-10 sm:w-12 sm:h-12 md:w-20 md:h-20 rounded-md overflow-hidden bg-muted flex-shrink-0 flex items-center justify-center"
                        className="rounded-md object-contain"
                        icon={CategoryIconComponent ? CategoryIconComponent : undefined}
                      />
                    <p className="font-medium text-xs sm:text-sm md:text-sm text-center truncate w-full">{category.name}</p>
                  </Card>
                </DialogTrigger>
              </Dialog>
            );
